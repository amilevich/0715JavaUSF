...
public class Account implements Serializable {
	...
	private static List<Account> accounts = new ArrayList<Account>(); // instantiates an object of list of accounts
																		// (empty right now)

	private Map<String, String> login = new HashMap<String, String>();
	private String account_id;
	private double balance;
	static Logger log = Logger.getLogger(Account.class.toString());

	public Account(Application app, double initialDeposit) { // takes the information from Application
		log.info("Opened account " + app.getAccount_id() + " with an initial deposit of $" + initialDeposit);
	...
	public void deposit(double amount) {
		...
		log.info("Amount deposited " + amount);
	}
	...
	public double withdraw(double amount) {
		if (amount <= this.balance && amount >= 0) {
			this.balance -= amount;
			log.info("Amount withdrawn " + amount);
			return amount;
		}
		System.out.println("Not enough in your account to withdraw this amount.");
		return 0;
	}
	...
	public void transfer(double amount, Account otherAccount) {
		if (amount <= this.balance && amount >= 0) {
			...
			log.info("Amount transfered " + amount);
		} else {
			System.out.println("Not enough in your account to transfer this amount.");
		}
	}
	...
	public String toString() {
		return "Account [login=" + login + ", account_id=" + account_id + ", balance=" + balance + "]"; // returns the
																										// string value
																										// of the
																										// object's
																										// fields
