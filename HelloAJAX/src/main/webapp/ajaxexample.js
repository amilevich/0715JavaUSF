 ...
 * XML is a data format so is JSON
 * XML - it is universal
 * JSON - JavaScript Object Notation -> human and machine readable. 
 * 
 * AJAX - is not a programming language nor a framework.
...
function getPokemon(){
	...
	xhttp.onreadystatechange = function(){
		...
		if( xhttp.readyState == 4 && xhttp.status == 200){
			...
			//if steps 4 and 5 are successful, we get our pokemon object
			setValues(pokemon);
		}
...
function setValues(pokemon){
	//step six, seven: JS not only reads it but also utilizing DOM manipulation
